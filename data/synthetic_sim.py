...
class NoForcesSim(object):
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        with np.errstate(divide='ignore'):
            ...
            return loc, vel, np.zeros((self.n_balls, self.n_balls))
...
class SpringChargeSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5, noise_var=0.,
                 ...
                 uniform_draw=True, noise_type = 'Normal'):
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            # Add noise to observations
            # noise_var = 0 therefore no noise (as desired for our investigations of computational and physical errors.)
            if self.noise_type == 'Normal':
                loc += np.random.randn(T_save, 2, self.n_balls) * self.noise_var
                vel += np.random.randn(T_save, 2, self.n_balls) * self.noise_var
            else:
                if self.noise_type == 'Uniform':
                    loc += np.random.random_sample((T_save, 2, self.n_balls)) * self.noise_var
                    vel += np.random.random_sample((T_save, 2, self.n_balls)) * self.noise_var
                else:
                    if self.noise_type == 'Pink':
                        loc, vel = powerlawnoise(loc, vel, -1, T, self._delta_T)
                    else:
                        if self.noise_type == 'Brownian':
                            loc, vel = powerlawnoise(loc, vel, -2, T, self._delta_T)
                        else:
...
class SpringChargeSimPCG(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5, noise_var=0.,
                 spring_interaction_strength=.1,
                 charge_interaction_strength=.2,
                 spring_types=[0., 0.5, 1.],
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        if self.uniform_draw:
            ...
            total_num_edges = int(0.5 * self.n_balls * (self.n_balls - 1))
            # randomly selects number of non-zero edges
            rg = rn.Generator(PCG64())
            num_edges = rg.integers(0, total_num_edges)
            # num_edges = random.randint(0, total_num_edges)
            edges = [0 for i in range(total_num_edges)]
            for i in range(num_edges):
                edges[i] = 1
            ...
            rg = rn.Generator(PCG64())
            spring_edges = rg.choice(self.spring_types,
                                            # self.spring_types is an array of relative spring strengths eg. [0., 0.5, 1.]
                                            size=(self.n_balls, self.n_balls),
                                            p=self.spring_prob)
            # spring_edges = np.random.choice(self.spring_types,
            #                                 # self.spring_types is an array of relative spring strengths eg. [0., 0.5, 1.]
            #                                 size=(self.n_balls, self.n_balls),
            #                                 p=self.spring_prob)  # prob. of each spring type
            spring_edges = np.tril(spring_edges) + np.tril(spring_edges, -1).T  # this makes the edges matrix symmetric
            ...
            # Sample charge edges
            charges = rg.choice(self.charge_types, size=(self.n_balls, 1), p=self.charge_prob)
            # charges = np.random.choice(self.charge_types, size=(self.n_balls, 1), p=self.charge_prob)
            charge_edges = charges.dot(charges.transpose())
            # np.fill_diagonal(charge_edges, 0)                    # remove self loops
        ...
        loc = np.zeros((T_save, 2, n))
        vel = np.zeros((T_save, 2, n))
        loc_next = rg.standard_normal(size= (2, n)) * self.loc_std
        vel_next = rg.standard_normal(size = (2, n))
        # loc_next = np.random.randn(2, n) * self.loc_std  # randn samples from a unit normal dist.
        # vel_next = np.random.randn(2, n)
        v_norm = np.sqrt((vel_next ** 2).sum(axis=0)).reshape(1, -1)
        vel_next = vel_next * self.vel_norm / v_norm
