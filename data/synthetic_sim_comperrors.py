...
class SpringChargeSim(object):
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        np.fill_diagonal(diag_mask, 0)
        counter = 0
        # stores the MSE's for each samplefrequecy and the current delta_T
        delta_T_array = []
        loc_mse_array = []
        vel_mse_array = []
        ...
        # for a set of different sample frequencies and delta_T's calculate the integration steps
        sample_frequency_original = sample_freq
        delta_T_orginal = self._delta_T
        for i in [1, 2, 4, 8, 16, 24, 32, 40, 48, 56, 64, 82, 100]:
            sample_frequency = i
            delta_T = delta_T_orginal * sample_frequency_original / sample_frequency
            delta_T_array.append(delta_T)
            counter = 0
            T_i = int(T * delta_T_orginal / delta_T)
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                ...
                loc_mse_array.append(mse_loc)
                vel_mse_array.append(mse_vel)
            # delta_T along dim = 0 corresponds to loc and vel_mse_array along dim 0
        return loc_mse_array, vel_mse_array, delta_T_array


    def sample_trajectory_animation(self, T=10000, sample_freq=10):
        n = self.n_balls
        assert (T % sample_freq == 0)
        T_save = int(T / sample_freq - 1)
        diag_mask = np.ones((n, n), dtype=bool)
        ...
        for i in [1, 2, 4, 8, 16, 24, 32, 40, 48, 56, 64, 82, 100]:
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                ...
                loc_array.append(loc_new)
        from trajectory_plot import draw_lines_animation
        draw_lines_animation(loc_array, animationtype='comperrors')
