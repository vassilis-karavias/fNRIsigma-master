...
class SpringSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5,
                 interaction_strength=.1, noise_var=0., delta_T = 0.001):
        ...
        # changing delta_T to allow us to test what the simulator error is - how it varies over integration stepsize
        # this will hopefully give us a better understanding of our error outputs.
        if (delta_T<=0):
            print("delta_T must be greater than 0. Using default delta_T = 0.001")
            self._delta_T = 0.001
        else:
...
class SpringChargeSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5, noise_var=0.,
                 ...
                 charge_prob=[0, 0.5, 0.5], delta_T = 0.001,
                 uniform_draw=True):
        ...
        if (delta_T<=0):
            ...
            self._delta_T = delta_T
        # fixed max F- want same simulations even when changing delta_T
        self._max_F = 0.1 / self._delta_T
    ...
    def sample_trajectory(self, savepath, plotcurrent = False, T=10000, sample_freq=10, sigmatest= False, animation= False):
        n = self.n_balls
        assert (T % sample_freq == 0)
        T_save = int(T / sample_freq - 1)
        diag_mask = np.ones((n, n), dtype=bool)
        ...
        # for the original delta_T and sampling frequency calculate trajectories (these will be used as the ground truth
        # for other delta_T components)
        # Initialize location and velocity, random starting point
        loc = np.zeros((T_save, 2, n))
        vel = np.zeros((T_save, 2, n))
        loc_next = np.random.randn(2, n) * self.loc_std  # randn samples from a unit normal dist.
        vel_next = np.random.randn(2, n)
        v_norm = np.sqrt((vel_next ** 2).sum(axis=0)).reshape(1, -1)
        vel_next = vel_next * self.vel_norm / v_norm
        # want to start the tests at the same point
        loc_fixed = copy.deepcopy(loc_next)
        vel_fixed = copy.deepcopy(vel_next)
        loc[0, :, :], vel[0, :, :] = self._clamp(loc_next, vel_next)
        ...
        sigma_1 = [0, 0.00001, 0.00005, 0.0001,0.0005, 0.001, 0.0016, 0.002, 0.00707, 0.01, 0.05, 0.10]
        for i in sigma_1:
            loc_mse_persigma = []
            vel_mse_persigma = []
            for j in range(10):
                counter = 0
                # Initialize location and velocity for new delta_T
                loc_new = np.zeros((T_save, 2, n))
                vel_new = np.zeros((T_save, 2, n))
                loc_next = copy.deepcopy(loc_fixed)
                vel_next = copy.deepcopy(vel_fixed)
                sigma = i
                # Gaussian sampled pertrubation in the initial conditions
                perturbations_pos = np.random.normal(loc = 0.0, scale = sigma, size= (len(loc_next), len(loc_next[0])))
                perturbations_vel = np.random.normal(loc=0.0, scale=sigma, size=(len(loc_next), len(loc_next[0])))
                loc_next += perturbations_pos
                vel_next += perturbations_vel
                loc_new[0, :, :], vel_new[0, :, :] = self._clamp(loc_next, vel_next)
                ...
                with np.errstate(divide='ignore', invalid='ignore'):
                    ...
                    for k in range(1, T):
                        ...
                        if k % sample_freq == 0:
                            loc_new[counter, :, :], vel_new[counter, :, :] = loc_next, vel_next
                            counter += 1
                        ...
                        # Add noise to observations
                    loc_new += np.random.randn(T_save, 2, self.n_balls) * self.noise_var
                    vel_new += np.random.randn(T_save, 2, self.n_balls) * self.noise_var
                    np.fill_diagonal(charge_edges, 0)
                    edges = np.concatenate( (np.expand_dims(spring_edges,0), np.expand_dims(charge_edges,0) ), axis=0)
                    if (j % 5 ==0) and plotcurrent:
                        from trajectory_plot import draw_lines
                        import os
                        fig = plt.figure(figsize=(7, 7))
                        ax = fig.add_axes([0, 0, 1, 1])
                        xmin_t, ymin_t, xmax_t, ymax_t = draw_lines(np.expand_dims(np.transpose(loc, (2,0,1)), axis = 0 ), 0, linestyle=':', alpha=0.6)
                        xmin_o, ymin_o, xmax_o, ymax_o = draw_lines(np.expand_dims(np.transpose(loc_new, (2,0,1)), axis = 0), 0, linestyle='-')

                        ax.set_xlim([min(xmin_t, xmin_o), max(xmax_t, xmax_o)])
                        ax.set_ylim([min(ymin_t, ymin_o), max(ymax_t, ymax_o)])
                        ax.set_xticks([])
                        ax.set_yticks([])
                        plt.savefig(os.path.join(savepath, 'trajectory'+ str(i) + str(j) + '.png'))
                        plt.show()

                        # now we have the new locations and velocities obtained using the same interaction graph with different
                        # integration steps. We must now find the MSE between the current value and the original also keep track
                        # of delta_T
                    mse_loc = ((loc_new-loc) ** 2).mean(axis = 2)/ self.n_balls
                    mse_vel = ((vel_new-vel) ** 2).mean(axis = 2)/ self.n_balls
                    loc_mse_persigma.append(mse_loc)
                    vel_mse_persigma.append(mse_vel)
                # average over the number of internal loops
            loc_mse_array.append(np.asarray(loc_mse_persigma).mean(axis = 0)/10)
            vel_mse_array.append(np.asarray(vel_mse_persigma).mean(axis = 0)/10)
        return loc_mse_array, vel_mse_array, sigma_1

    def sample_trajectory_animation(self, savepath, plotcurrent = False, T=10000, sample_freq=10):
        n = self.n_balls
        assert (T % sample_freq == 0)
        T_save = int(T / sample_freq - 1)
        diag_mask = np.ones((n, n), dtype=bool)
        ...
        # loc, vel gives the locations and velocities of the particles in this step
            ...
            # Initialize location and velocity for new delta_T
            location_array = []
            perturbationinposx = np.linspace(-0.01,0.01,10)
            perturbationinposy = np.linspace(-0.01,0.01,10)
            for i in range(5):
                for j in range(3):
                    loc_new = np.zeros((T_save, 2, n))
                    vel_new = np.zeros((T_save, 2, n))
                    ...
                    vel_next = copy.deepcopy(vel_fixed)
                    counter = 0
                    for m in range(len(loc_next)):
                        for k in range(len(loc_next[m])):
                            loc_next[m,k] = loc_next[m,k] + perturbationinposx[random.randint(0,len(perturbationinposx)-1)]
                            vel_next[m,k] = vel_next[m, k] + perturbationinposx[
                                random.randint(0, len(perturbationinposx) - 1)]
                    loc_new[0, :, :], vel_new[0, :, :] = self._clamp(loc_next, vel_next)
                    ...
                    with np.errstate(divide='ignore', invalid='ignore'):
                        ...
                        vel_new += np.random.randn(T_save, 2, self.n_balls) * self.noise_var
                        np.fill_diagonal(charge_edges, 0)
                        edges = np.concatenate((np.expand_dims(spring_edges, 0), np.expand_dims(charge_edges, 0)),
                                               axis=0)
                        loc_new = np.transpose(loc_new,(2,0,1))
                        location_array.append(loc_new)
            from trajectory_plot import draw_lines_animation
            draw_lines_animation(location_array, animationtype='chaostheory')

class SpringChargeQuadSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5, noise_var=0.,
                 spring_interaction_strength=.1, 
                 charge_interaction_strength=.2,
                 quad_interaction_strength=.1, delta_T = 0.001 ):
...
class SpringChargeFspringSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5, noise_var=0.,
                 ...
                 fspring_length=1., delta_T = 0.001):
