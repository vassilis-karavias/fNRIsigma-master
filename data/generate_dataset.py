...
if args.sim_type == 'springcharge':
    sim = SpringChargeSim(noise_var=0.3, n_balls=args.n_balls, box_size=5.0, noise_type = args.noise_type)
...
elif args.sim_type == 'noforces':
    sim = NoForcesSim(noise_var=0.0, n_balls=args.n_balls, box_size=5.0)
...
# generate data for many different timesteps (sample frequencies)
if args.timesteptest:
    initialsamplingfreq = args.sample_freq
    for i in [8, 16, 32, 64, args.sample_freq]:
        sample_freq = i
        print("Generating {} training simulations".format(args.num_train))
        loc_train, vel_train, edges_train = generate_dataset(args.num_train, args.length, sample_freq)
        currentpath = os.path.join(args.savefolder, 'datasamplefreq' + str(i))
        os.makedirs(currentpath)

        np.save(os.path.join(currentpath,'loc_train.npy'), loc_train)
        np.save(os.path.join(currentpath,'vel_train.npy'), vel_train)
        np.save(os.path.join(currentpath,'edges_train.npy'), edges_train)

        print("Generating {} validation simulations".format(args.num_valid))
        loc_valid, vel_valid, edges_valid = generate_dataset(args.num_valid, args.length, args.sample_freq)

        np.save(os.path.join(currentpath,'loc_valid.npy'), loc_valid)
        np.save(os.path.join(currentpath,'vel_valid.npy'), vel_valid)
        np.save(os.path.join(currentpath,'edges_valid.npy'), edges_valid)

        print("Generating {} test simulations".format(args.num_test))
        loc_test, vel_test, edges_test= generate_dataset(args.num_test, args.length_test, args.sample_freq)

        np.save(os.path.join(currentpath,'loc_test.npy'), loc_test)
        np.save(os.path.join(currentpath,'vel_test.npy'), vel_test)
        np.save(os.path.join(currentpath,'edges_test.npy'), edges_test)
else:
    print("Generating {} training simulations".format(args.num_train))
